...
import { UUID } from 'crypto';
  ...
  async findOne(@Param('id') id: number): Promise<Product> {
    ...
    const user = await this.ProductService.findOneResto({ where: { id } });
    if (!user) {
  ...
  async update(
    ...
    @Post(':id')
    async addcateg(@Param('id') id: number, @Body('card') card: any): Promise<Product> {
      ...
      return this.ProductService.save(product);
        ...
        async deletecateg(@Param('id') id: number ): Promise<void> {
          //handle the error if resto not found
          const items: any = await this.ProductService.findOneResto({ where: { id } });
          if (!items) {
            throw new Error('resto not found');
          }
    ...
    async additems(@Param('id') id: number, @Body('card') card: any,@Body('idCategorie') idCategorie:string): Promise<Product> {
      ...
      Product.card.categories[idCategorie] = {...Product.card.categories[idCategorie],...card};
